...
void Application::bufer_read_students(Vector<Studentas> &studentai)
{
    ...
    cout << "Atlikta atminties perskirstymu: " << allocCounter << endl;
...
{
    auto sortStart = hrClock::now();
    std::copy_if(kietiakai.begin(), kietiakai.end(), std::back_inserter(vargsai), isVargsas);
    kietiakai.erase(std::remove_if(kietiakai.begin(), kietiakai.end(), isVargsas), kietiakai.end());
    cout << "Studentu dalinimo i dvi grupes laikas: " << durationDouble(hrClock::now() - sortStart).count() << " s" << endl;
}
...
void Application::runAutoMode()
{
    ...
    if (container_type == 1)
    {
        ...
        std::sort(kietiakai.begin(), kietiakai.end(), compare);
        std::sort(vargsai.begin(), vargsai.end(), compare);
        otherTimer.Start();
